...
class BPSOccupancyGrid2dDecoder01(nn.Module):
    ...
    def __init__(self, num_basis_points: int, feature_dim: int, scene_dims: Tuple[int, int], num_conv_layers: int = 4):
        ...
        self.fc_start = nn.Linear(self.input_dim, self.init_channels * self.initial_spatial_dim * self.initial_spatial_dim)
        self.bn_start = nn.BatchNorm1d(self.init_channels * self.initial_spatial_dim * self.initial_spatial_dim)
    ...
    def forward(self, bps_encoded_scene: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(-1, self.init_channels, self.initial_spatial_dim, self.initial_spatial_dim)
