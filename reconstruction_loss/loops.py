...
def train_bps_grid_decoder_2d(
	...
	checkpoint_path: str = os.path.abspath("./checkpoints")
	checkpoint_filename_prefix: str = f"grid_pts{num_basis_points}_enc{encoding_type}_norm{norm_bound_shape}_bs{batch_size}"
	...
	for epoch in range(num_epochs):
		...
		# validation
		avg_val_loss = evaluate_bps_grid_decoder(decoder, val_dataloader)
		print(f"[Epoch {epoch:02d}/{(num_epochs-1):02d}] Avg Train Loss: {avg_train_loss:.4f} | Avg Validation Loss: {avg_val_loss:.4f}")
...
def evaluate_bps_grid_decoder(
    ...
    with torch.no_grad():
        for bps_encoding, target_grid in dataloader:
            predicted_grid: torch.Tensor = decoder(bps_encoding)
            target_grid = target_grid.to(predicted_grid.dtype)
            batch_loss: torch.Tensor = criterion(predicted_grid, target_grid, reduction='mean')
...
def train_bps_cloud_decoder_2d(
	...
	checkpoint_path: str = os.path.abspath("./checkpoints")
	checkpoint_filename_prefix: str = f"cloud_pts{num_basis_points}_enc{encoding_type}_norm{norm_bound_shape}_bs{batch_size}"
	...
	for epoch in range(num_epochs):
		...
		# validation
		avg_val_loss = evaluate_bps_cloud_decoder(decoder, val_dataloader)
		print(f"[Epoch {epoch:02d}/{(num_epochs-1):02d}] Avg Train Loss: {avg_train_loss:.4f} | Avg Validation Loss: {avg_val_loss:.4f}")
